xml.users(:type => "array") do
  ...
  @users.each do |user|
    xml.user do
      ...
      xml.blood(user.blood, :type => "integer")
      xml.sex(user.sex, :type => "integer")
      ...
      xml.created_at(user.created_at.strftime("%Y-%m-%dT%H:%M:%S"), :type => "datetime")    
      xml.updated_at(user.updated_at.strftime("%Y-%m-%dT%H:%M:%S"), :type => "datetime")        
    end
  end
end