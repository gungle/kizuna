xml.tweets(:type => "array") do
  ...
  @tweets.each do |tweet|
    xml.tweet do
      ...
      xml.tweet_data(tweet.tweet, :type => "text")
      ...
      xml.created_at(tweet.created_at.strftime("%Y/%m/%d %H:%M:%S"), :type => "datetime")    
      xml.updated_at(tweet.updated_at.strftime("%Y/%m/%d %H:%M:%S"), :type => "datetime")        
    end
  end
end